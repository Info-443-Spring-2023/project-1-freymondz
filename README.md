...
We choose to use `src/features/HomePage/HomePage2.tsx` as our architectural element to assess. There are several code smells that we identified in this component.

- Commented out code that is no longer used.
- In general inconsistent formatting.
- Bad variable names.
- Redundant conditional statements.
- Redundant React components.
- Doesn't adhere to any JavaScript style guide.
- Incorrect usage of React `useEffect` and event listeners.
- Difficulty in testing the component.

The component itself should be relatively simple to allow for easy testing and for future modification. However because of the duplication of defined TSX and confusing conditional statements, it can be difficult for someone new to quickly understand what the component does, understand it's edge cases, and make any changes.
...
We choose to write these tests because the `HomePage2` component itself is fairly simply as it acts as a container for other components. The component itself doesn't have any complex logic and is mostly responsible for rendering other components. We just have to ensure that the state of other components is correctly handled when the user interacts with the `HomePage2` component. We also wanted to test the responsiveness of the home page to ensure that it renders correctly on different screen sizes.
...
We used prettier to standardize the styling/formatting of the code. Double quotes instead of single quotes, using semicolons, consistent spacing, etc. We also removed commented out code and cleaned up redundant conditional statements. Variable names were standardized to camel case and the control flow for the home page was simplified.

Instead of copying and pasting the filter bar and filter button for each size of the screen we used variables and ternary operators to determine the state of the filter button and whether to render the filter bar or not.

During the writing of our tests we also identified and fixed an issue with our event listener and `useEffect()`. Originally the event listener was added to the window every time the component was rendered. This caused the event listener to be called multiple times when the window was resized. We fixed this by adding a cleanup function to `useEffect()` that removes the event listener when the component is unmounted.

To make it easier to test the responsiveness of the `HomePage2` component we added a `test-id` attribute to the `Stack` mui component to make it easier to select the component in our unit tests.
